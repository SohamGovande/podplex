...
def train_model(config: ModelConfig):
    ...
    # balance by size using the sample 
    import fairscale
    from fairscale.nn.pipe.balance import balance_by_size
    partitions = torch.cuda.device_count()
    # balance = balance_by_size(partitions, model, sample['encoder_input'].to(device))
    ...
    optimizer = OSS(model.parameters(), optim=torch.optim.Adam)
    ...
    for epoch in range(initial_epoch, config.num_epochs):
        ...
        batch_iterator = tqdm(train_dataloader, desc=f"Processing Epoch {epoch:02d}", disable=False)
